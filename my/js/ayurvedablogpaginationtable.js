$(document).ready(function(){
            $('#postdata').after('<div id="nav"></div>');
            var rowsShown = 15;
            var rowsTotal = $('#postdata tbody tr').length;
            var numPages = rowsTotal/rowsShown;
            for(i = 0;i < numPages;i++) {
                var pageNum = i + 1;
                $('#nav').append('<a href="#" rel="'+i+'">'+pageNum+'</a> ');
            }
            $('#postdata tbody tr').hide();
            $('#postdata tbody tr').slice(0, rowsShown).show();
            ...
            $('#nav a').bind('click', function(){
                ...
                $('#postdata tbody tr').css('opacity','0.0').hide().slice(startItem, endItem).
...
// this function creates a three-column table and adds it to the screen
   ...
   tocTable += '<table id="postdata" width="100%" class="" border="0" cellpadding="5" cellspacing="5">';
   tocTable += '<thead>';
   tocTable += '<tr>';
   ...
   for (var i = 0; i < postTitle.length; i++) {
      if (filter == '') {
         tocTable += '<tbody><tr><td class="toc-entry-col1"><a href="' + postUrl[i] + '" title="' + postSum[i] + '">' + postTitle[i] + '</a></td><td class="toc-entry-col2">' + postDate[i] + '</td><td class="toc-entry-col3">' + postLabels[i] + '</td></tr></tbody>';
         numDisplayed++;
      } else {
   ...
   if (numDisplayed == postTitle.length) {
      var tocNote = '<span class="toc-note">Displaying posts<br/></span>'; }
   else {
      var tocNote = '<span class="toc-note">Displaying posts labeled \'';
